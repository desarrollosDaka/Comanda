...
const updateMasterOrderAndDetails = async (req, res) => {
  try {
    ...
    // Eliminar todos los pagos existentes para el id_detalleº
    await sequelize.models.modelOrdersPay.destroy({
      where: { ID_detalle: idOrder },
    });
    ...
    if (order && client) {
      ...
      res
        .status(400)
        .json({ message: "Error al actualizar la orden o el cliente" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error interno del servidor" });
